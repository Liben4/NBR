...
export const CategoryView: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-8 py-8 min-h-[70vh] transition-colors duration-200">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <select
              ...
              className="px-3 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 text-slate-800 dark:text-slate-200 text-xs focus:outline-none focus:border-blue-600 shadow-sm"
            >
      ...
      {sortedArticles.length === 0 ? (
        <div className="text-center py-20 text-slate-500">
          ...
          <button 
            ...
            className="mt-4 px-4 py-2 rounded-xl bg-blue-600 text-white text-xs font-semibold shadow"
          >
            Return to Homepage
          </button>
        </div>
        ...
        <div className="space-y-10">
          ...
          {leadArticle && (
            <div 
              ...
              className="group cursor-pointer rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 overflow-hidden shadow-md dark:shadow-2xl transition-all duration-300 grid grid-cols-1 lg:grid-cols-12 gap-0"
            >
              <div className="lg:col-span-7 relative aspect-video lg:aspect-auto w-full overflow-hidden bg-slate-200 dark:bg-slate-800">
                <img 
              ...
              <div className="lg:col-span-5 p-6 sm:p-8 flex flex-col justify-between bg-gradient-to-b from-white to-slate-50 dark:from-slate-900 dark:to-slate-950">
                <div>
                  <h2 className="font-editorial text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-tight">
                    {leadArticle.title}
                  </h2>
                  <p className="font-sans text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed mt-3 line-clamp-4">
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                  <div className="flex items-center gap-2.5">
                    <img 
                      ...
                      referrerPolicy="no-referrer"
                    />
                    <span className="font-medium text-slate-700 dark:text-slate-200">{leadArticle.author.name}</span>
                  </div>
          ...
          {otherArticles.length > 0 && (
            <div>
              <h3 className="font-brand text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-4">
                  ...
                  >
                    <div>
                      ...
                      <div className="p-4 sm:p-5">
                        <h4 className="font-editorial text-lg font-bold text-slate-900 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors line-clamp-2 leading-snug">
                          {article.title}
                        </h4>
                        <p className="font-sans text-xs text-slate-600 dark:text-slate-400 mt-2 line-clamp-2">
                          {article.subtitle}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="px-4 pb-4 sm:px-5 sm:pb-5 pt-3 border-t border-slate-100 dark:border-slate-800/60 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                      <span className="text-[11px] truncate max-w-[120px] text-slate-700 dark:text-slate-300">
                        {article.author.name}
                      </span>
