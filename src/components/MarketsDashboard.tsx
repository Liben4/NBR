...
export const MarketsDashboard: React.FC = () => {
  ...
  return (
    <section className="w-full bg-slate-100/70 dark:bg-slate-900/60 border-y border-slate-200 dark:border-slate-800/80 py-8 px-4 sm:px-8 my-6 transition-colors duration-200">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
          ...
          <button 
            ...
            className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold border border-slate-300 dark:border-slate-700 shadow-sm transition-colors"
          >
            ...
            return (
              <div
                ...
                className={`p-3.5 rounded-xl border cursor-pointer transition-all duration-200 flex flex-col justify-between ${
                  isSelected
                    ? 'bg-blue-50/80 dark:bg-blue-950/40 border-blue-500 shadow-sm shadow-blue-500/20 ring-1 ring-blue-500/50'
              ...
              >
                <div>
                  ...
                  <div className="mt-1 flex items-baseline justify-between gap-1">
                    <span className="text-base sm:text-lg font-bold text-slate-900 dark:text-slate-100 font-sans tracking-tight">
                      {item.value}
                    </span>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 bg-white dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-4 sm:p-6 shadow-sm">
          
          {/* Left / Chart Area (8 cols) */}
          <div className="lg:col-span-8 flex flex-col justify-between">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
              <div>
                <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-blue-100 dark:bg-blue-600/20 text-blue-800 dark:text-blue-400 border border-blue-200 dark:border-blue-500/30">
                  {selectedIndicator.category} Indicator
                </span>
                <h3 className="font-editorial text-lg sm:text-xl font-bold text-slate-900 dark:text-slate-100 mt-1">
                  {selectedIndicator.name}
                </h3>
              </div>

              {/* Timeframe selector */}
              <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-1 rounded-lg text-xs">
                ...
                <AreaChart data={selectedIndicator.history}>
                  <defs>
                    <linearGradient id="colorValue" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#3b82f6" stopOpacity={isDarkMode ? 0.4 : 0.25} />
          ...
          <div className="lg:col-span-4 flex flex-col justify-between border-t lg:border-t-0 lg:border-l border-slate-200 dark:border-slate-800/80 pt-4 lg:pt-0 lg:pl-6">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-2">
                <Coins className="w-4 h-4 text-amber-600 dark:text-amber-400" />
                <h4 className="font-brand text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200">
              ...
              {currencies.slice(0, 5).map((cur) => (
                <div key={cur.code} className="py-2 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="font-bold text-slate-800 dark:text-slate-200">{cur.code}</span>
            ...
            <button
              ...
              className="w-full py-2 px-3 rounded-lg bg-blue-50 dark:bg-blue-600/20 hover:bg-blue-100 dark:hover:bg-blue-600/30 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-500/30 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors"
            >
