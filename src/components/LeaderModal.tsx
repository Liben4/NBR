...
export const LeaderModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn">
      <div 
        className="relative w-full max-w-2xl bg-slate-900 border border-slate-700 rounded-3xl p-6 sm:p-8 shadow-2xl overflow-y-auto max-h-[90vh]"
      ...
      >
        ...
        {/* Top Profile Header */}
        <div className="flex flex-col sm:flex-row gap-5 items-start sm:items-center pb-6 border-b border-slate-800">
          <img 
            src={selectedLeader.avatar} 
            alt={selectedLeader.name}
            className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl object-cover border-2 border-blue-500/50 shadow-xl"
            referrerPolicy="no-referrer"
          />

          <div>
            <span className="px-2.5 py-0.5 rounded text-[11px] font-bold uppercase tracking-wider bg-amber-500/10 text-amber-400 border border-amber-500/20 inline-block mb-1.5">
              {selectedLeader.sector}
            </span>
            <h2 className="font-editorial text-2xl sm:text-3xl font-bold text-slate-100">
              {selectedLeader.name}
            </h2>
        ...
        {/* Executive Bio */}
        <div className="py-5">
          <h4 className="font-brand text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">
        ...
        <div className="py-4 border-t border-slate-800">
          <h4 className="font-brand text-xs font-bold uppercase tracking-widest text-slate-400 mb-3 flex items-center gap-1.5">
        ...
        {selectedLeader.quote && (
          <div className="mt-4 p-4 rounded-2xl bg-slate-950 border-l-4 border-blue-500 text-xs text-slate-300 italic font-editorial leading-relaxed">
