      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {opinionArticles.map((article: Article) => (
          <article
            ...
            className="group cursor-pointer rounded-2xl bg-white dark:bg-gradient-to-b dark:from-slate-900 dark:via-slate-900/90 dark:to-slate-950 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 p-6 flex flex-col justify-between shadow-sm hover:shadow-lg transition-all duration-300 relative overflow-hidden"
          >
            ...
            <div>
              <div className="flex items-center gap-3.5 pb-4 border-b border-slate-100 dark:border-slate-800/80">
                <div className="relative">
                  ...
                  />
                  <span className="absolute -bottom-1 -right-1 p-0.5 rounded-full bg-white dark:bg-slate-950 text-amber-600 dark:text-amber-400 border border-slate-200 dark:border-slate-800 shadow-sm">
                    <Feather className="w-3 h-3" />
                  </span>
                </div>

                <div>
                  <h4 className="font-bold text-slate-900 dark:text-slate-200 text-sm group-hover:text-blue-600 dark:group-hover:text-amber-300 transition-colors">
                    {article.author.name}
                  </h4>
              ...
              {/* Headline */}
              <h3 className="font-editorial text-lg sm:text-xl font-bold text-slate-900 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors mt-4 leading-snug">
                {article.title}
              </h3>

              {/* Subtitle / Excerpt */}
              <p className="font-sans text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed mt-2.5 line-clamp-3">
                {article.subtitle || article.excerpt}
              </p>

              {/* Pull quote callout if available */}
              {article.pullQuote && (
                <div className="mt-4 p-3 rounded-xl bg-slate-50 dark:bg-slate-950/80 border-l-2 border-amber-500 text-xs text-slate-700 dark:text-slate-300 italic font-editorial">
            ...
            {/* Footer */}
            <div className="mt-6 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-xs text-slate-500">
