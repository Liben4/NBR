...
export const LatestNewsGrid: React.FC = () => {
  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 sm:px-8 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-4 border-b border-slate-800 mb-6">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-2 justify-between md:justify-end">
          ...
          {/* Grid / List Switcher */}
          <div className="hidden sm:flex items-center bg-slate-900 border border-slate-800 p-0.5 rounded-lg text-slate-400">
            <button
            ...
            <button
              ...
              className={`p-1.5 rounded ${viewMode === 'list' ? 'bg-slate-800 text-white' : 'hover:text-white'}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredArticles.map((article: Article) => (
            <article
              ...
              className="group cursor-pointer rounded-2xl bg-slate-900/90 hover:bg-slate-850 border border-slate-800 hover:border-slate-700 shadow-lg hover:shadow-xl transition-all duration-300 flex flex-col justify-between overflow-hidden"
            >
              <div>
                {/* Thumbnail Image */}
                ...
                <div className="p-4 sm:p-5">
                  <h3 className="font-editorial text-lg sm:text-xl font-bold text-slate-100 group-hover:text-blue-400 transition-colors line-clamp-2 leading-snug">
                    {article.title}
                  </h3>

                  <p className="font-sans text-xs sm:text-sm text-slate-400 mt-2.5 line-clamp-2 leading-relaxed">
        ...
        <div className="divide-y divide-slate-800 bg-slate-900/60 border border-slate-800 rounded-2xl overflow-hidden">
          {filteredArticles.map((article: Article) => (
            <div
              key={article.id}
              onClick={() => openArticle(article)}
              className="p-4 sm:p-5 hover:bg-slate-800/80 transition-colors cursor-pointer flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between group"
            >
              <div className="flex gap-4 items-center flex-1 min-w-0">
                <div className="w-20 h-16 sm:w-24 sm:h-20 rounded-lg overflow-hidden shrink-0 bg-slate-800">
                  <img 
                    src={article.featuredImage} 
                    alt={article.title}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="font-editorial text-base sm:text-lg font-bold text-slate-200 group-hover:text-blue-400 transition-colors truncate">
                    {article.title}
                  </h3>
              ...
              <div className="flex items-center gap-4 shrink-0 text-xs text-slate-400 self-end sm:self-center">
                <span className="hidden lg:inline text-slate-400">{article.author.name}</span>
