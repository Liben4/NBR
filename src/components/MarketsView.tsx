...
export const MarketsView: React.FC = () => {
  const { marketIndicators, currencies, setCurrentView, isDarkMode } = useApp();
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-8 py-8 min-h-[80vh] transition-colors duration-200">
      ...
      <div className="pb-6 border-b border-slate-200 dark:border-slate-800 mb-8">
        <button
          ...
          className="inline-flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 transition-colors mb-3"
        >
        ...
        <h1 className="font-editorial text-3xl sm:text-4xl md:text-5xl font-bold text-slate-900 dark:text-slate-100 mt-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-10">
        ...
        <div className="lg:col-span-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-md dark:shadow-xl flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800 mb-5">
              ...
              <span className="text-[11px] text-emerald-700 dark:text-emerald-400 font-bold uppercase tracking-wide bg-emerald-50 dark:bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-200 dark:border-emerald-500/20">
                Market Floating Rate
              </span>
            </div>

            {/* Direction Switcher */}
            <div className="flex items-center justify-between bg-slate-100 dark:bg-slate-950 p-1 rounded-xl border border-slate-200 dark:border-slate-800 mb-4 text-xs">
              <button
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 font-bold text-lg focus:outline-none focus:border-blue-600"
                  />
                  <span className="absolute right-4 top-3.5 text-xs text-slate-500 dark:text-slate-400 font-bold">
                  ...
                  {currencies.map(cur => (
                    <button
                      ...
                      className={`p-2.5 rounded-xl border text-left flex items-center gap-2 transition-all ${
                        selectedCurrencyCode === cur.code
                          ? 'bg-blue-50 dark:bg-blue-600/30 border-blue-500 text-blue-900 dark:text-white'
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <span className="font-bold text-xs block text-slate-800 dark:text-slate-200">{cur.code}</span>
            ...
            {/* Result Box */}
            <div className="mt-6 p-4 rounded-2xl bg-gradient-to-r from-blue-50 via-slate-50 to-blue-100/30 dark:from-blue-950/60 dark:to-slate-950 border border-blue-200 dark:border-blue-500/30">
              <span className="text-[11px] text-slate-600 dark:text-slate-400 block mb-1">
                Estimated Commercial Value
              </span>
              <div className="text-2xl sm:text-3xl font-extrabold font-sans text-amber-600 dark:text-amber-400">
                {conversionDirection === 'foreignToEtb' ? `${convertedValue} ETB` : `${convertedValue} ${selectedCurrencyCode}`}
              </div>
              <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 mt-2 pt-2 border-t border-slate-200 dark:border-slate-800">
        ...
        <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-md dark:shadow-xl flex flex-col justify-between">
          <div>
            <div className="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-slate-100 dark:border-slate-800 mb-4">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-wider text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-600/10 px-2 py-0.5 rounded border border-blue-200 dark:border-blue-500/20">
                  {selectedIndicator.category} Indicator
                </span>
                <h3 className="font-editorial text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100 mt-1">
                  {selectedIndicator.name}
                </h3>
              </div>

              <div className="text-right">
                <span className="text-2xl font-extrabold text-slate-900 dark:text-slate-100 font-sans">
                  {selectedIndicator.value}
                </span>
                <span className={`text-xs font-bold block ${
                  selectedIndicator.isPositive ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'
                }`}>
            ...
            <div className="h-64 sm:h-72 w-full pt-3">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={selectedIndicator.history}>
                  ...
                  <XAxis dataKey="date" stroke={isDarkMode ? '#64748b' : '#94a3b8'} fontSize={11} tickLine={false} axisLine={false} />
                  <YAxis stroke={isDarkMode ? '#64748b' : '#94a3b8'} fontSize={11} tickLine={false} axisLine={false} domain={['auto', 'auto']} />
                  <Tooltip 
                    contentStyle={{ 
          ...
          <div className="mt-4 p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400">
            <span className="font-semibold text-slate-800 dark:text-slate-300">Analysis: </span>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-md dark:shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100 dark:border-slate-800 mb-6">
          <div>
            ...
            <h3 className="font-editorial text-2xl font-bold text-slate-900 dark:text-slate-100 mt-1">
              Ethiopian Securities Exchange (ESX) Listings Pipeline
            </h3>
          </div>
          <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">Addis Ababa Securities Registry</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div 
              key={idx}
              className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 flex flex-col justify-between shadow-sm"
            >
              <div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-900 flex items-center justify-between text-xs">
